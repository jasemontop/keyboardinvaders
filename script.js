...
async function submitAccount() {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/${accountMode === "register" ? "register" : "login"}`, {
      method: "POST",
    ...
    if (!response.ok) {
      throw new Error(data.error || "Could not connect to commander network.");
    }
    ...
    if (accountMode === "login" && data.player) {
...
function showAnnouncement(small, main, duration = 2800) {
  ...
  box.style.setProperty("--announce-duration", `${Math.max(1800, duration)}ms`);
