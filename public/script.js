...
let lastRemoteCoins = null;
let lastLocalAccountChangeAt = 0;
...
function markLocalAccountChange() {
  lastLocalAccountChangeAt = Date.now();
}
...
async function pollLiveAccountAndLeaderboard() {
  if (!accountToken || guestMode) return;

  try {
    // Never pull remote balance into an active run or immediately after a local purchase.
    if (!running && Date.now() - lastLocalAccountChangeAt > 3500) {
      ...
      if (meResponse.ok) {
        ...
        const remoteCoins = Number(meData.player?.coins);

        if (Number.isFinite(remoteCoins)) {
          if (lastRemoteCoins === null) lastRemoteCoins = remoteCoins;

          if (remoteCoins !== coins) {
            const difference = remoteCoins - coins;
            ...
            if (difference > 0) {
              showGameNotification(
                "🪙 COINS RECEIVED",
                `+${difference.toLocaleString()} coins added to your account.`,
                "coins"
              );
              playSuccessSound();
            } else {
              showGameNotification(
                "🪙 BALANCE UPDATED",
                `${Math.abs(difference).toLocaleString()} coins removed.`,
                "info"
              );
            }
          } else {
            lastRemoteCoins = remoteCoins;
          }
        }
      }
    }
      ...
      if (leaderboardLeaders) {
        ...
        Object.keys(currentLeaders).forEach(key => {
          ...
          if (next && previous && next !== previous) {
            showGameNotification(
              "👑 NEW #1 PLAYER!",
              `${next} just took #1 in ${labels[key]}.`,
              "crown"
            );
          }
        });
...
function startLiveUpdates() {
  ...
  pollLiveAccountAndLeaderboard();
  liveUpdateTimer = setInterval(pollLiveAccountAndLeaderboard, 7000);
}
...
.addEventListener(
  ...
  () => {
    ...
    if (accountToken) {
      setTimeout(pollLiveAccountAndLeaderboard, 400);
    }
